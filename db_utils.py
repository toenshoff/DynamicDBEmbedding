...
class Database:
    ...
    def get_row_val_graph(self, add_row_classes=True, partition=1):
        ...
        partition_map = self.tuple_partition.copy()
            ...
            if (not col_id == self.predict_col) and self.tuple_partition[row_id] <= partition:
                ...
                for val in values:
                    ...
                    if not G.has_node(val_id):
                        ...
                        partition_map[val_id] = partition_map[row_id]
                    else:
                        partition_map[val_id] = min(partition_map[row_id], partition_map[val_id])

                    G.add_edge(row_id, val_id)

        return G, partition_map
