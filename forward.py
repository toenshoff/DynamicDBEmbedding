...
#"""
kernels_dict = {'Population@city': lambda x, y: np.exp(- ((x - y) ** 2) / (2 * ((0.05 * np.maximum(np.abs(x), np.abs(y)) + eps) ** 2))),
                'SurfaceArea@country': lambda x, y: np.exp(- ((x - y) ** 2) / (2 * ((0.05 * np.maximum(np.abs(x), np.abs(y)) + eps) ** 2))),
                'IndepYear@country': lambda x, y: np.exp(- ((x - y) ** 2) / (2 * ((0.05 * np.maximum(np.abs(x), np.abs(y)) + eps) ** 2))),
                'Population@country': lambda x, y: np.exp(- ((x - y) ** 2) / (2 * ((0.05 * np.maximum(np.abs(x), np.abs(y)) + eps) ** 2))),
                'GNP@country': lambda x, y: np.exp(- ((x - y) ** 2) / (2 * ((0.05 * np.maximum(np.abs(x), np.abs(y)) + eps) ** 2))),
                'GNPOld@country': lambda x, y: np.exp(- ((x - y) ** 2) / (2 * ((0.05 * np.maximum(np.abs(x), np.abs(y)) + eps) ** 2)))}
#"""
#kernels_dict = {}
...
class Forward(torch.nn.Module):
    ...
    def __init__(self, dim, num_schemes, row_idx, scheme_idx):
        ...
        self.row_idx_dict = row_idx
        self.row_idx = np.vectorize(lambda x: self.row_idx_dict[x])
    ...
    def append(self, x, row_id):
        self.row_idx_dict[row_id] = self.num_tuples
        self.num_tuples += 1

        x = torch.tensor(x, device=self.x.device).view(1, -1)
        self.x = torch.nn.Parameter(torch.cat([self.x, x], dim=0))
